...
node *context;
uint *b2link;   // link is in conflict with unistd.h, we renamed it to b2link
RLEInfo *rleInfo;
uint first[65536];
uint last[65536];
...
void findMatches() {
  ...
  while (get >= 0) {
	...
	scn = b2link[scn]; // Table65535 lookup
      };

      first[cur] = b2link[first[cur]]; // Waste first entry
	    ...
	    bool isRelocated)
{
  ...
  b2link = (uint*)malloc(sizeof(uint) * ibufSize);
