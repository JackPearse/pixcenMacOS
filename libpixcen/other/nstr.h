...
#endif
	void empty(void){(*this)=0;} // void empty(void){(*this)=NULL;}
...
private:
    ...
    static void copyconvert( nstrptr<char> *p, const wchar_t *s){if(wcstombs(p->b,s,p->l+1)==-1)static_cast<void>(*p->b=0),p->l=0;}
    static void copyconvert( nstrptr<wchar_t> *p, const char *s){if(mbstowcs(p->b,s,p->l+1)==-1)static_cast<void>(*p->b=0),p->l=0;}
