...
#include <stdlib.h> //deprecated: #include <malloc.h>
...
void C64Interface::paralell_for_ycell(int from, int to, Function &&fn)
{
    ...
    if (cells)
    {
        ...
        for (int r = 0; r < s; r++)
        {
            ...
            w[r] = g_pThreadPool->schedule([f, t, &fn]
            {
                for (int r = f; r < t; r++)
                {
                    fn(r);
...
void C64Interface::Save(LPCTSTR pszFileName, LPCTSTR type)
{
    ...
    /*
    // Should be handled by host application.
    ...
    */
	...
	FILE *fp=fopen(pszFileName,_T("wb"));
	if(!fp)
...
void C64Interface::RenderImage(CImage &inimg, int startx, int starty, int width, int height)
{
	...
	g_pThreadPool->paralell_for(ymax - starty, [starty, startx, xmax, &img, pw, this](int py)
	{
		int y = py + starty;
		int px, x;
...
void C64Interface::Load(nmemfile &file, LPCTSTR type, int version)
{
	if(!lstrcmpi(_T("gpx"),type))
	{
		...
		else
		{
            ...
            BYTE *temp_buffer = (BYTE *)aligned_alloc(temp_buffersize, 16);
...
void C64Interface::Create(int mapsize, int colorsize, int screensize, int backbuffers)
{
    ...
    /*
    buffer = (BYTE *)aligned_alloc(buffersize, 16);
    tbuffer = (BYTE *)aligned_alloc(buffersize, 16);
     */
    buffer = (BYTE *)malloc(buffersize);
    tbuffer = (BYTE *)malloc(buffersize);
...
{
	nmemfile file;
	FILE *fp= fopen(pszFileName,_T("rb"));
	if(!fp)
	...
	,parent(pp)
{
    ...
    g_pThreadPool->paralell_for(ysize, [this, prgbmap](int y)
	{
...
void C64Interface::ImportHelper::ReduceColors(int maxcolor, BYTE *force, int forcecount)
{
    ...
    g_pThreadPool->paralell_for(ysize / ycs, [this, maxcolor, force, forcecount](int cy)
	{
