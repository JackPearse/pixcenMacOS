...
#include "platform.h" // threadpool is defined here!
#include "ImageFast.h"
#include "C64Col.h"
#include "narray.h"
#include "nstr.h"
...
public:
	...
	virtual BYTE GetPixelColourMap(int x, int y) = 0;
	virtual void SetPixel(int x, int y, BYTE color) = 0;
...
public:
	// C64 RLE codec functions (exposed for PixcenFoundation bridge)
...
protected:
	...
	void GetSaveFormats(narray<autoptr<SaveFormat>,int> &fmt) override;
	void Save(nmemfile &file, LPCTSTR type) override;
	void Load(nmemfile &file, LPCTSTR type, int version) override;
...
private:
#ifdef __THREAD_WINDOWS__
	CEvent quit;
	HANDLE thread;
#endif
#ifdef __THREAD_LINUX__
	...
	virtual int GetCharCount(void) override {return charcount;}
#ifdef __THREAD_WINDOWS__
